...
dfm::pt::node dfm::pt::parse::obj_param_name(const std::vector< dfm::lexer::token_type >& tokens, const size_t pos)
{
    static const dfm::pt::rule_type RULE = "OBJ_PARAM_NAME := <WS>* <DELPHI_NAME> <WS>*";
    __TOKEN_DEBUG(__PRETTY_FUNCTION__, tokens, pos, RULE);
    __TOKEN_RANGE_CHECK(__PRETTY_FUNCTION__, tokens, pos, RULE);

    dfm::pt::node n;
    {
        n.type = dfm::pt::ntypes::OBJ_PARAM_NAME;
        n.rule = RULE;
