...
dfm::pt::node dfm::pt::parse::value_set(const std::vector< dfm::lexer::token_type >& tokens, const size_t pos)
{
    static const dfm::pt::rule_type RULE = "VALUE_SET := <WS>* '[' <WS>* <VALUE_SET_VALUES> <WS>* ']' <WS>*";
    __TOKEN_DEBUG(__PRETTY_FUNCTION__, tokens, pos, RULE);
    __TOKEN_RANGE_CHECK(__PRETTY_FUNCTION__, tokens, pos, RULE);

    dfm::pt::node n;
    {
        n.type = dfm::pt::ntypes::VALUE_SET;
        n.rule = RULE;
        ...
        n.children.push_back(dfm::pt::parse::value_set_values(tokens, i));
        i += n.children.back().offset();
        i += dfm::pt::parse::consume_whitespace(n, tokens, i);
