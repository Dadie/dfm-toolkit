...
dfm::pt::node dfm::pt::parse::obj_type(const std::vector< dfm::lexer::token_type >& tokens, const size_t pos)
{
    static const dfm::pt::rule_type RULE = "OBJ_TYPE := <WS>* <DELPHI_NAME> <WS>*";
    __TOKEN_DEBUG(__PRETTY_FUNCTION__, tokens, pos, RULE);

    dfm::pt::node n;
    {
        n.type = dfm::pt::ntypes::OBJ_TYPE;
        n.rule = RULE;
