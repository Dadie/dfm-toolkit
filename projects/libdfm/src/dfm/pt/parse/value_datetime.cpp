...
// Datetime as float
static dfm::pt::node value_datetime_1(const std::vector< dfm::lexer::token_type >& tokens, const size_t pos)
{
    ...
    dfm::pt::node n;
    {
        ...
        constexpr const char* regex_str = "(-|\\+)?(([0-9]+\\.[0-9]*)|([0-9]*\\.[0-9]+))d";
        const std::regex regexpr { regex_str };
        if (!std::regex_match(tokens.at(pos), regexpr))
        {
            throw dfm::pt::parse_error(RULE, tokens.at(pos), pos, dfm::pt::ntypes::VALUE_DATETIME);
        }
...
// Datetime as integer
static dfm::pt::node value_datetime_2(const std::vector< dfm::lexer::token_type >& tokens, const size_t pos)
{
    static const dfm::pt::rule_type RULE = "VALUE_DATETIME := ('-'|'+')? [0-9]+ 'd'";
    __TOKEN_DEBUG(__PRETTY_FUNCTION__, tokens, pos, RULE);
    __TOKEN_RANGE_CHECK(__PRETTY_FUNCTION__, tokens, pos, RULE);
    ...
    dfm::pt::node n;
    {
        ...
        constexpr const char* regex_str = "(-|\\+)?[0-9]+d";
        const std::regex regexpr { regex_str };
        if (!std::regex_match(tokens.at(pos), regexpr))
        {
            throw dfm::pt::parse_error(RULE, tokens.at(pos), pos, dfm::pt::ntypes::VALUE_DATETIME);
...
dfm::pt::node dfm::pt::parse::value_datetime(const std::vector< dfm::lexer::token_type >& tokens, const size_t pos)
{
    ...
    {
        fns.push_back(&value_datetime_1);
        fns.push_back(&value_datetime_2);
    }
