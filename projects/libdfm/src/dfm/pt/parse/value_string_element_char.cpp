...
static dfm::pt::node value_string_element_char_1(const std::vector< dfm::lexer::token_type >& tokens, const size_t pos)
{
    static const dfm::pt::rule_type RULE = "VALUE_STRING_ELEMENT_CHAR := '#'[0-9]+";
    __TOKEN_DEBUG(__PRETTY_FUNCTION__, tokens, pos, RULE);
    __TOKEN_RANGE_CHECK(__PRETTY_FUNCTION__, tokens, pos, RULE);

    dfm::pt::node n;
    {
        n.type = dfm::pt::ntypes::VALUE_STRING_ELEMENT_CHAR;
        n.rule = RULE;
        ...
        {
            throw dfm::pt::parse_error(RULE, tokens.at(pos), pos, dfm::pt::ntypes::VALUE_STRING_ELEMENT_CHAR);
        }
...
static dfm::pt::node value_string_element_char_2(const std::vector< dfm::lexer::token_type >& tokens, const size_t pos)
{
    static const dfm::pt::rule_type RULE = "VALUE_STRING_ELEMENT_CHAR := '#$'[0-9A-Fa-f]+";
    __TOKEN_DEBUG(__PRETTY_FUNCTION__, tokens, pos, RULE);
    __TOKEN_RANGE_CHECK(__PRETTY_FUNCTION__, tokens, pos, RULE);
    ...
    dfm::pt::node n;
    {
        ...
        constexpr const char* regex_str = "#\\$[0-9A-Fa-f]+";
        const std::regex regexpr { regex_str };
        if (!std::regex_match(tokens.at(pos), regexpr))
        {
            throw dfm::pt::parse_error(RULE, tokens.at(pos), pos, dfm::pt::ntypes::VALUE_STRING_ELEMENT_CHAR);
...
dfm::pt::node dfm::pt::parse::value_string_element_char(
    const std::vector< dfm::lexer::token_type >& tokens, const size_t pos)
{
    ...
    {
        fns.push_back(value_string_element_char_1);
        fns.push_back(value_string_element_char_2);
    }
