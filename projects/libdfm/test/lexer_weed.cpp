...
int lexer_weed(int argc, char** argv)
{
    if (argc < 2)
    {
        return EXIT_FAILURE;
    ...
    const std::filesystem::path out_path =
        dfm_in_path.parent_path().parent_path() / "exp" / "lexer" / dfm_in_path.stem().replace_extension(".weed.token");
