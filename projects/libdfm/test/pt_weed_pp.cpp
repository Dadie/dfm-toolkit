...
int pt_weed_pp(int argc, char** argv)
{
    if (argc < 2)
    {
        return EXIT_FAILURE;
    ...
    const std::filesystem::path out_path =
        dfm_in_path.parent_path().parent_path() / "exp" / "pt" / dfm_in_path.stem().replace_extension(".weed.pp");
    ...
    if (cfg.generate_files)
    {
        if (cfg.allow_overwrite || !std::filesystem::exists(out_path))
        {
            ...
            root_weed_pt.pp(0, ofs);
        }
    }

    std::stringstream sstr;
    {
        root_weed_pt.pp(0, sstr);
    }
